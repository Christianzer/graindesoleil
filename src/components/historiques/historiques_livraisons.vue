<template>
  <div class="container-fluid p-3">
    <PageHeader title="Historiques livraisons" subtitle="Bons de livraison du client" crumb="Suivi" />
        ...
        <template v-else>
          ...
          >
            ...
            <template v-slot:cell(montant_total_ttc)="row">
              {{new Intl.NumberFormat().format(row.item.montant_total_ttc)}} FCFA
            </template>
            ...
            <template v-slot:cell(actions)="row">
              <b-button
                  ...
                  @click="consulter_livraison(row.item.code_commande)"
              >
               Consulter
              </b-button>
...
export default {
  ...
  methods: {
    ...
    async consulter_livraison(code_demande){
      this.$router.push({ name: 'livraison', params: { code_commande: code_demande}})
    }
  }
}
</script>
