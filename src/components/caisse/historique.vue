<template>
  <div class="container-fluid p-3">
    <PageHeader title="Historiques encaissement" subtitle="Suivi des encaissements par client" crumb="Suivi">
      <template #actions>
        ...
        <template v-else>
          ...
          >
            ...
            <template v-slot:cell(actions)="row">
              ...
              >
                Consulter ses encaissements
              </b-button>
              <b-button
                  size="sm"
                  class="mr-1"
                  variant="outline-success"
                  @click="factures(row.item)"
              >
...
export default {
  ...
  methods: {
    ...
    async consultation(id) {
      this.$router.push({name:'historiques_encaissement_clients', params: { id: id }})
    },
    async factures(data) {
